...
- **VAC/Game-баны** — у валидных тянется профиль, бейдж `VAC BAN` / `GAME BAN` + дни (отключается в настройках)
...
Сессия свежая на аккаунт (как `requests.Session` в оригинале — куки живут только внутри пары запросов, WinHTTP-автоjar отключён), таймауты 12с. Каждый HTTP-запрос (включая страницы профилей для банов) берёт слот у глобального планировщика — параллельные залпы исключены.
